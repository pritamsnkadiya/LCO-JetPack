...
public class ProfileViewModel extends Observable implements View.OnClickListener {
    ...
    public ProfileViewModel(Context context, Activity activity, ActivityDashboardBinding binding, String user_id, String is_mso, String is_lco, String is_isd, String is_lba) {
        ...
        binding.toolbarMenuId.toolbarTitle.setText("User Profile");
        binding.toolbarMenuId.civBackBtn.setOnClickListener(this);
        binding.llShowList.setOnClickListener(this);
        binding.fab.setOnClickListener(this);
        binding.toolbarMenuId.imgSearch.setVisibility(View.GONE);

        ((ApplicationAppContext) activity.getApplication()).getNetworkComponent().inject(ProfileViewModel.this);

        if (is_mso.equalsIgnoreCase("1") || is_lco.equalsIgnoreCase("1") || is_isd.equalsIgnoreCase("1")) {
            getUserDetails(user_id);
        } else if (is_lba.equalsIgnoreCase("1")) {
            getUserDetails(user_id);
        } else {
            getLbaUserDetails(user_id);
        }
    }
    ...
    public void getUserDetails(String user_id) {
        ...
        Call<ResponsModel> responseModelCall = mService.getUserDetails(user_id);
    ...
    public void getLbaUserDetails(String user_id) {
        ApiInterface mService = retrofit.create(ApiInterface.class);
        Call<ResponsModel> responseModelCall = mService.getLbaUserDetails(user_id);
        responseModelCall.enqueue(new Callback<ResponsModel>() {
            @Override
            public void onResponse(Call<ResponsModel> call, Response<ResponsModel> response) {
                if (response.isSuccessful()) {
                    ...
                    if (response.body().result.image == null) {
                        ...
                        Glide.with(context)
                                .load(response.body().result.image.trim())
                                .into(binding.imgView);
                    }
                    id = response.body().result.id;
                    name = response.body().result.name;
                    user_type = response.body().result.user_type;
                    ...
                    binding.tvSubscription.setText(response.body().subscription);
                    binding.tvNoUsers.setText(response.body().user);
                    if (response.body().result.plans.size() > 0) {
                        for (int i = 0; i < response.body().result.plans.size(); i++) {
                            channelsList.add(response.body().result.plans.get(i));
                        }
                    }
                } else {
                }
